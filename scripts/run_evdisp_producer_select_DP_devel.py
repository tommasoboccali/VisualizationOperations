...
def main():
    # main starts here
    global MODCFG,DRYRUN,RAW2DIGISEQ,RECOSEQ,INPUTCFG,GLOBALTAG,SCENARIO
...
def runproducer(instance):
    global SCENARIO,DRYRUN,RAW2DIGISEQ,RECOSEQ,KILLING,AREA,INPUTCFG,GLOBALTAG,STREAM,TRIGGERTYPE,L1TTSEL,HLTSEL,SELININPUT,MODCFG,CONFIGFILE,TYPE
...
#
        if INPUTCFG=="None" or GLOBALTAG=="None" or STREAM=="None":
        # read from DAS- update to production level DAS
           dasinfo=commands.getoutput("wget  --no-check-certificate 'http://cmsweb.cern.ch/tier0/express_config?run=&stream=Express' -o /dev/null -O /dev/stdout")
           if dasinfo=="[]":
              dasinfo=commands.getoutput("wget  --no-check-certificate 'http://cmsweb.cern.ch/tier0/express_config?run=&stream=ExpressCosmics' -o /dev/null -O /dev/stdout")
           dasinfo=dasinfo.replace("[","")
           dasinfo=dasinfo.replace("]","")
           exec("daslist="+dasinfo)

        GTFROMDAS=""
        if GLOBALTAG=="None":
            GLOBALTAG=daslist['global_tag']
            GTFROMDAS="(From DAS)="

        SCENARIOFROMDAS=""
        if SCENARIO=="None":
            SCENARIO=daslist['scenario']
...
#            time.sleep(1)
        ...
        print " I have to sleep for 200 seconds; thanks for your patience..."
        time.sleep(200)
